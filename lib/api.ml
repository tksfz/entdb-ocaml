...
module Make (S : Storage.S) = struct
        ...
        try
          ...
          let definition =
            Data.Entity_definition.
              {
                id = Data.Entity_definition.create_id ();
                name;
                description;
          ...
          | Ok () -> Lwt.return (Ok ())
          | Error e -> Lwt.return (Error (Storage.error_to_string e))
  ...
  let list_entity_definitions t =
    ...
    | Error e -> Lwt.return (Error (Storage.error_to_string e))
    | Ok defs ->
        let to_json (def : Data.Entity_definition.t) : Yojson.Safe.t =
          let base : (string * Yojson.Safe.t) list =
            [
              ("id", `String (Type_id.to_string def.id));
    ...
    | json -> (
        ...
        | Ok (Some def) -> (
            ...
            | Some id_str -> (
                ...
                | Ok type_id ->
                    if String.equal (Type_id.prefix type_id) def.type_id_prefix then
                      let data = Data.Entity_data.{ id = type_id; entity_definition_id = def.id; data = json } in
                      ...
                      Lwt.return
                        (Error
                           (Printf.sprintf "ID prefix '%s' does not match entity definition prefix '%s'"
                              (Type_id.prefix type_id) def.type_id_prefix)))))
  ...
  let get_entity_data t id_str =
    ...
    | Ok type_id -> (
        ...
        | Ok (Some data) -> Lwt.return (Ok (Some data.Data.Entity_data.data)))
end
